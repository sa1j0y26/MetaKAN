#!/usr/bin/env python3
"""
Run MetaKAN function_fitting baselines and HyperKAN sweeps.
...
def build_runs(args: argparse.Namespace, meta_root: Path) -> List[RunSpec]:
        ...
        RunSpec(
            ...
            command=[
                ...
                *base_common,
                ...
                str(args.lr_kan),
                "--kan_bspline_grid",
                str(args.kan_bspline_grid),
                "--kan_bspline_order",
                str(args.kan_bspline_order),
                "--kan_shortcut_name",
                args.kan_shortcut_name,
            ],
    ...
    runs.append(
        RunSpec(
            ...
            model="HyperKAN",
            dataset=args.dataset,
            command=[py, "train_hyper.py", "--model", "HyperKAN", *hyper_base],
            meta={
                "type": "baseline",
                "model": "HyperKAN",
    ...
    # HyperKAN sweep: hidden_dim
    if args.hyperkan_hidden_dims:
        for hd in args.hyperkan_hidden_dims:
            runs.append(
                RunSpec(
                    run_id=next_id("sweep"),
                    group="hyperkan_hidden_dim",
                    model="HyperKAN",
                    dataset=args.dataset,
                    command=[
                        py,
...
def main() -> int:
    ...
    parser.add_argument("--dataset", type=str, default="I.6.20b")
    parser.add_argument("--layers-width", type=int, nargs="+", default=[5, 5, 5])
    ...
    parser.add_argument("--kan-bspline-grid", type=int, default=5)
    parser.add_argument("--kan-bspline-order", type=int, default=3)
    parser.add_argument("--kan-shortcut-name", type=str, default="silu")
    ...
    parser.add_argument("--hyperkan-hidden-dims", type=int, nargs="*", default=[8, 16, 32, 64])
    ...
    runs_path = out_dir / "runs_function_fitting.jsonl"
    ...
    for run in runs:
        ...
        run_record.update({"cwd": str(meta_ff)})
        ...
        proc = subprocess.run(run.command, cwd=str(meta_ff))
