#!/usr/bin/env python3
"""
Plot function_fitting baseline comparison and HyperKAN sweep from runs JSONL.
"""
...
def plot_baseline(runs: List[Dict[str, Any]], out_dir: Path) -> None:
    ...
    plt.ylabel("Test metric (lower is better for MSE)")
    plt.title("MLP vs KAN vs HyperKAN")
    plt.tight_layout()
    out_path = out_dir / "ff_baseline_compare.png"
    plt.savefig(out_path, dpi=150)
    plt.close()


def plot_hidden_dim_sweep(runs: List[Dict[str, Any]], out_dir: Path) -> None:
    sweep = [r for r in runs if r.get("group") == "hyperkan_hidden_dim"]
    if not sweep:
        return
    ...
    plt.xlabel("HyperKAN hidden_dim")
    plt.ylabel("Test metric (lower is better for MSE)")
    plt.title("HyperKAN hidden_dim sweep")
    plt.tight_layout()
    out_path = out_dir / "ff_hyperkan_hidden_dim_sweep.png"
    plt.savefig(out_path, dpi=150)
    plt.close()
...
def main() -> int:
    ...
    parser.add_argument(
        "--runs",
        type=str,
        default=str(Path(__file__).parent / "out" / "runs_function_fitting.jsonl"),
    )
